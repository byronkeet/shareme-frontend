...
const Pin = ({ pin }) => {
	...
	const { postedBy, image, _id, destination, save, about } = pin;

	const user = fetchUser();

	const alreadySaved = !!(save?.filter((item) => item?.postedBy?._id === user?.sub).length > 0);
	...
	return (
		<div className='m-2'>
			...
			>
				{image && (
				<img className='rounded-lg w-full' src={(urlFor(image).width(250).url())} alt={about ? `${about}` : 'user-post'} />
				)}
				{postHovered && (
					<div
